...
def plot_features(data, leakage_data, proper_data, plot: bool = False):
    if plot:
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10))
        ax1.plot(data.index, data["value"], label="Original Price", alpha=0.5)
        ax1.plot(
...
def plot_predictions(leakage_results, proper_results, plot: bool = False):
    if plot:
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 10))
        dates_leak, y_test_leak, y_pred_leak = leakage_results
        dates_proper, y_test_proper, y_pred_proper = proper_results
        ...
        ax1.plot(dates_leak, y_test_leak, label="Actual", alpha=0.7)
        ax1.plot(
            dates_leak, y_pred_leak, "--", label=f"Leakage (MAPE: {mape_leak:.2f}%)"
        )
        ax1.plot(
            dates_proper,
            y_pred_proper,
...
if __name__ == "__main__":
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))
    m1, d1, y1, p1 = evaluate_model(
        data_lookahead, ["future_5day_ma", "future_volatility"], "With Lookahead", ax1
    )
    m2, d2, y2, p2 = evaluate_model(
        data_correct, ["past_5day_ma", "past_volatility"], "Without Lookahead", ax2
    )
    plt.tight_layout()
    plt.show()
    ...
    combined = pd.concat(
        [
            japan_gas.set_index("date")["value"],
            em_gas.set_index("date")["value"],
            us_rate.set_index("date")["value"],
        ],
        axis=1,
    )
