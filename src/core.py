...
def train_model(X: np.ndarray, y: np.ndarray) -> Tuple[LinearRegression, Dict]:
    ...
    metrics = {'r2': r2_score(y, y_pred), 'rmse': np.sqrt(np.mean((y - y_pred) ** 2)), 'mae': np.mean(np.abs(y - y_pred))}
    return (model, metrics)

def plot_leakage_comparison(metrics_no_leakage: Dict, metrics_with_leakage: Dict, title: str, output_path: Path, plot: bool=False):
